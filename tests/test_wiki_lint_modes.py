...
def test_hook_exits_zero_and_reports_only_that_file(tmp_path, capsys):
    ...
    append_body(repo, "Wiki/sources/kernkonzept/quelle-eins.md", "\n[[nirgendwo]]\n")
    assert run(repo, "--hook", str(repo / "Wiki/concepts/concept/kohaerenz.md")) == 0
    out = capsys.readouterr().out
    assert "no-k-marker-outside-canon Wiki/concepts/concept/kohaerenz.md" in out and "nirgendwo" not in out


def test_hook_on_candidate_runs_the_write_time_subset(tmp_path, capsys):
    repo = make_wiki(tmp_path)
    page = write_page(repo, "Wiki/candidates/concepts/concept/kandidat.md",
                      {**CONCEPT_FRONT, "slug": "kandidat", "status": "draft", "kind_detail": "nope"},
                      CONCEPT_BODY + "\n[[nirgendwo]]\n")
    assert run(repo, "--hook", str(page)) == 0
    out = capsys.readouterr().out
    assert "enum Wiki/candidates/concepts/concept/kandidat.md" in out and "broken-link" not in out
...
def test_fix_writes_reverse_link_and_defaults(tmp_path, capsys):
    repo = make_wiki(tmp_path)
    page = remove_reverse_link(repo)
    set_front(repo, "Wiki/sources/kernkonzept/quelle-eins.md", truncated=None)
    set_front(repo, "Wiki/concepts/concept/kohaerenz.md", canon_status=None)
    run(repo, "--fix")
    ...
    assert wiki_pages.split_frontmatter((repo / "Wiki/sources/kernkonzept/quelle-eins.md").read_text())[0]["truncated"] is False
...
def test_suggest_lists_orphan_mentions(tmp_path, capsys):
    ...
    run(repo, "--suggest")
    out = capsys.readouterr().out
    assert "orphan Wiki/concepts/concept/einsam.md: link [[einsam]] from Wiki/sources/kernkonzept/quelle-eins.md" in out
