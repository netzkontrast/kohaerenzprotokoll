...
3. [scripts/lit_critic_gate.py](../../scripts/lit_critic_gate.py) — dekidierbare Kapitel-Lints ([lint_chapter.py](../../scripts/lint_chapter.py), lexikalisch, kostenlos, `--locks-only`) und lit-critic's seven editorial lenses against [tools/lit-critic/CANON.md](../../tools/lit-critic/CANON.md) and [STYLE.md](../../tools/lit-critic/STYLE.md). Reports land in [Plan/quality/lit-critic/](../../Plan/quality/lit-critic/). Only `critical` findings block; the `horizon` lens never does. See the [lit-critic skill](./lit-critic/SKILL.md).
