...
ones — now C13. Document 17 did the same twice: C14, whose third-person side
stood on `aegis`, and C15, five rows on four pages.
...
## Two sessions reading one document — the merge of 2026-09-25

This branch read documents 16 and 17 while another session read document 16 and
main took that reading (pull request #85), with this branch's C13 (pull request
#86). **The rule was that main's merged reading is the wiki's**: every page,
record, run artifact and judgement of document 16 came from main, this branch's
list stayed as a blind re-reading, and document 17's readings were applied again
to main's pages, one commit per page. Its pre-classification against main's wiki
was the same as against this branch's — 847 candidates, 508 by lookup, 345 to
judgement — because reconciliation by lookup reads surfaces, and both readings
of document 16 had left the same surfaces. What moved were the words around the
readings: a finding that said which rows a page held had to be checked against
main's page (C14), and judgement numbers had to be the ones main's ledger gives
(J82, J84 in place of this branch's J80–J81).
