...
`Plan/runs/judgements.jsonl` holds 91 <!--state:judgements.total--> records,
...
77 <!--state:pairs.labelled-->. `pairs.py rows()` calls `trainset.surface_pairs()` directly
...
This repository's largest trainset is 77 <!--state:pairs.labelled--> labelled
...
(14/17 = 82%); `fold()` now decides 43 <!--state:pairs.fold_correct--> of
77 <!--state:pairs.labelled-->, and the docstring dates its first number and
...
- **`MIPROv2`, `BootstrapFewShotWithRandomSearch`** — refused. They want
  100+ and 50+ examples; this repository's largest trainset is 77
