...
`Plan/runs/judgements.jsonl` holds 84 <!--state:judgements.total--> records,
...
71 <!--state:pairs.labelled-->. `pairs.py rows()` calls `trainset.surface_pairs()` directly
...
This repository's largest trainset is 71 <!--state:pairs.labelled--> labelled
...
71 <!--state:pairs.labelled-->, and the docstring dates its first number and
...
- **`MIPROv2`, `BootstrapFewShotWithRandomSearch`** — refused. They want
  100+ and 50+ examples; this repository's largest trainset is 71
