...
`score_rule("fold")`: **73 <!--state:pairs.labelled--> labelled pairs; `fold()`
decides 42 <!--state:pairs.fold_correct--> of them (57%).** `trainset.py`'s own
...
one, `rule:plural`, which decides 50 <!--state:pairs.plural_correct-->
...
42 <!--state:pairs.fold_correct--> of 73
<!--state:pairs.labelled--> pairs for free, the plural rule of decision 010
decides 50 <!--state:pairs.plural_correct-->, and `pairs.py` sends a model only
...
- **A judge for job 1 or job 2** — waits on 20–50 hand-labelled cases per the
  calibration protocol's own floor. Job 1's 73 <!--state:pairs.labelled-->
