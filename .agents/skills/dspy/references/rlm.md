...
- The academic idea `dspy.RLM` implements: "Recursive Language Models" (Zhang,
  ...
  variable in a code environment instead of as prompt text (`dspy:dspy/predict/rlm.py:1-9`).
- **`dspy.RLM`**, the DSPy 3.3.1 module this file documents, used by
  ...
  without warning." into its own docstring (`dspy:dspy/predict/rlm.py:115`,
  `dspy:dspy/utils/annotation.py:53-56`).
...
- **`dspy_skills`** loads `.agents/skills/` the way Claude Code does:
  ...
  `SKILL.md`. `briefing()` calls it (`scripts/rlm_ingest.py:155-161`), so the
...
- **`scripts/read.py`** supplies the document already prefixed `NNN| `
  (`numbered()`, `scripts/rlm_ingest.py:164-169`). Left to count for itself the
...
(`scripts/rlm_ingest.py:352-359`). The default model is free and answers a
...
empty response (`scripts/rlm_ingest.py:95-100`).
...
(`scripts/rlm_ingest.py:243-245`). Unlike every other model-calling step here,
...
(`scripts/rlm_ingest.py:242-253`). It therefore never writes an `lmrun`
...
| field | what it means |
|---|---|
| `written_by:` | `dspy.RLM, model <m>, <n> iterations, <s>s — <verdict>`. The verdict clause is `judge()`'s string. `scripts/state.py:364-384`'s `trainset.gold_candidate_lists` measure reads a `written_by:` line to tell a reader's list from a reconstruction — but only on files named `03-candidates.md` (below), so this file is excluded from that glob before the line is ever read |
| `ran:` | today's date |
| `verified:` | tier 1 — `good` of `total` candidates whose cited line, checked by the same comparison `quotes.py` uses, actually contains the term (`verified()`, `scripts/rlm_ingest.py:225-237`) |
| `reach:` | tier 2 — the furthest verified citation's line as a share of the document's own span, and how many of ten equal-width "tenths" of that span at least one verified citation falls in (`reach()`, `scripts/rlm_ingest.py:143-152`) |
...
(`scripts/rlm_ingest.py:286-306`). `judge()` turns the three tiers into one
verdict, forced beating everything else (`scripts/rlm_ingest.py:216-222`).
...
`fold()` (`scripts/rlm_ingest.py:317-345`). It raises `SystemExit` if either
...
(`scripts/rlm_ingest.py:366-394`).

**What it may not do** (`scripts/rlm_ingest.py:38-46`):
...
(`dspy:dspy/primitives/python_interpreter.py:84-94`). Verified in this
...
any Deno `2.x` passes (`dspy:dspy/primitives/python_interpreter.py:39-41,124-138`).
...
deno_command.` (`dspy:dspy/primitives/python_interpreter.py:124-138`).
...
(`dspy:dspy/primitives/runner.js:3-4`). After that the Deno cache is warm and
...
cannot be shared across threads. ...")` (`dspy:dspy/primitives/python_interpreter.py:391`).

## The module

Every parameter `dspy.RLM(...)` takes, precisely (`dspy:dspy/predict/rlm.py:139-166`;
...
`f"{iteration+1}/{self.max_iters}"` (`dspy:dspy/predict/rlm.py:680`).
...
sub-call runs (`dspy:dspy/predict/rlm.py:263-274`). It does not count the
...
against `dspy:dspy/predict/rlm.py:261-325,543-562`).
...
(`dspy:dspy/predict/rlm.py:427-430,701`). `rlm_ingest.py` never passes one; it
...
(`dspy:dspy/predict/rlm.py:425-438`). This is the rule a `History`-carrying
...
(`dspy:dspy/predict/rlm.py:189-190,219-238`). `rlm_ingest.py`'s own names
...
(`dspy:dspy/predict/rlm.py:197-202`). **A tool is not sandboxed by the
...
(`scripts/rlm_ingest.py:115-140`), are read-only host-side lookups: each
...
(`dspy:dspy/predict/rlm.py:558-562,634-641,755`).
...
(`dspy:dspy/predict/rlm.py:359-369`):
...
- **`variables_info`** — one formatted block per input: name, type, optional
  ...
  (`dspy:dspy/primitives/repl_types.py:38-95`). Roughly 1 KB of every input —
...
on every step (`dspy:dspy/predict/rlm.py:654-664,694`). A name the model
...
(`dspy:dspy/primitives/runner.js:23,74-88`). It raises a `BaseException`
...
(`dspy:dspy/primitives/code_interpreter.py:32-48`); `_process_final_output`
...
(`dspy:dspy/predict/rlm.py:564-598`). Both failures go back as this step's
...
(`dspy:dspy/primitives/runner.js:342-346`), so `print('x')` followed by
...
|---|---|---|
| **forced final output** | `max_iters` runs out with no `SUBMIT`; `_extract_fallback` calls a second, code-free predictor (`extract`) over `variables_info` and `repl_history` and returns a normal-shaped `Prediction` with every declared field filled and `final_reasoning="Extract forced final output"` — the only tell. [checked: rlm-forced-final-output] | `dspy:dspy/predict/rlm.py:543-562,735-757` |
| **budget exceeded inside the sandbox** | over `max_llm_calls`, `RuntimeError("LLM call limit exceeded: ...")` is raised **inside** the sandbox; `_execute_code` catches it and turns it into `[Error] RuntimeError: [...]` in the history. The loop continues and the model can still `SUBMIT` — nothing stops the run from the outside | `dspy:dspy/predict/rlm.py:267-274,654-664` |
| **interpreter state leaks between documents** | a caller-owned interpreter, passed positionally, is never reset or shut down between calls: "may be reused sequentially with the same RLM instance, but must not be shared by overlapping invocations" | `dspy:dspy/predict/rlm.py:127-128,519-537` |
| **an input named `json`** | the default sandbox's variable injection rejects the literal key `"json"` with a terminal `CodeInterpreterError("Invalid variable name: 'json'")` — the module still constructs; only a call with that input name fails | `dspy:dspy/primitives/python_interpreter.py:695-696` |
...
| **History wrapper, DSPy 3.3.1** | 3.3.1 validates both directions; the same wrapper now fails at once with `ValueError: Unexpected inputs not declared in the signature: ['history']`, before any LM or sandbox work | `dspy:dspy/predict/rlm.py:425-438`; re-checked here 2026-09-24, `scripts/lmrun.py:28-32` |
| **experimental status** | `@experimental` injects "This class may change or be removed in a future release without warning" into the class docstring; nothing pins that it will not | `dspy:dspy/predict/rlm.py:115` |
...
(`scripts/rlm_ingest.py:209-213`) and the reason `judge()` checks it first,
ahead of the citation tiers (`scripts/rlm_ingest.py:216-222`).
...
(`Plan/concept/rlm-the-real-one_2026-09-17.md:57`) — a few thousand tokens,
...
(`Plan/concept/rlm-the-real-one_2026-09-17.md:67-78`). That corpus-scale loop
...
(`Plan/concept/rlm-measured-on-the-trap_2026-09-17.md:19-30`). The RLM run
...
(`Plan/concept/rlm-measured-on-the-trap_2026-09-17.md:34-39`). The note's own
...
1. **Cited line** — every candidate came back as `- term  ^[Lnn]`; a
   ...
   (`verified()`, `scripts/rlm_ingest.py:225-237`).
...
logs a warning each time (`dspy:dspy/primitives/module.py:336-348` per the
...
(`Plan/concept/rlm-the-real-one_2026-09-17.md:57-61`) — which is exactly the
...
- **It needs a real, running `dspy.RLM` instance to patch, and none has run
  yet.** Nothing here has called `dspy.RLM` against a real model (`NOW.md:85-87`
...
- **Hooks widen what a terminal error looks like.** DSPy's own `_execute_code`
  ...
  (`dspy:dspy/predict/rlm.py:661-664`); the hooks package's copy of the same
...
This is `check_dspy_skill.py`'s own probe (`scripts/check_dspy_skill.py:361-371`),
...
(`scripts/lm_fixture.py:36,116-133`). [checked: rlm-runs-offline]

**The gate is Deno, not the model.** The probe requires `import deno` to
succeed first, and reports `NotRun` — never "held" — if it cannot
(`scripts/check_dspy_skill.py:361-365`), per P15/P23: a probe that cannot run
...
model that never calls `SUBMIT` (`scripts/check_dspy_skill.py:374-387`,
`scripts/lm_fixture.py:63-80`).
...
| GEPA compiling the RLM itself | catalogued, not built | an RLM has exactly two predictors, `generate_action` and `extract`; GEPA on it would rewrite DSPy's own REPL template, not only a task instruction (`dspy-agent-skills:skills/dspy-rlm-module/SKILL.md:80-97`). Waits on the same gate as any real RLM run: "two or three more hand-read documents" before extraction is trainable (`Plan/concept/dspy-toolchain_2026-09-23.md:304-305`, `.claude/skills/tools/SKILL.md:230-231`) |
| A corpus-scale RLM loop ("idea E") | catalogued, not built | `llm_query_batched` over many documents for one term — "What does the corpus say about AEGIS?" is exactly the RLM case, sized at 315 documents / 41 sub-calls without reading anything (`Plan/concept/rlm-the-real-one_2026-09-17.md:67-78`). Explicitly "not worth doing before the hand pass finishes" (`:98-101`) |
| Making a term, not a document, the unit of work ("idea D") | rejected | "a term-first pass reads many documents through one lens, which is exactly the contamination the census exists to prevent. The unit stays the document" (`Plan/concept/rlm-transfer_2026-09-17.md:63-68`) |
| `SHOW_VARS()`-style self-report | catalogued, not built | telling a reader what the derived cache already knows, the way an RLM's REPL can inspect its own environment (`Plan/concept/rlm-the-real-one_2026-09-17.md:103-108`) |
