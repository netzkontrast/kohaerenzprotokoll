...
| `scripts/wiki_index.py` / `scripts/reconcile.py` | `fold()`'s own docstring claimed behaviour it did not have, repeated in two other files | `scripts/judgements.py` replays all 86 <!--state:judgements.total--> recorded near-match decisions against the current code and reports `agrees`/`DISAGREES`/`judgement` |
