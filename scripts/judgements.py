...
RENDERED = ROOT / "Plan" / "runs" / "judgements.md"


def render() -> int:
    """Write the ledger as markdown, because qmd cannot see the ledger.

    qmd indexes `**/*.md` and nothing else — 1047 files in the index, every one
    markdown, zero JSON. So `judgements.jsonl` and every `reconcile.json` are
    invisible to every search, which is the opposite of what they are for: they
    are the densest record of what has already been decided, and the thing most
    worth finding without re-reading.

    This is derived, never edited, and regenerated by `--render`. The `.jsonl`
    stays the source of truth: it is what `replay()` reads and what `trainset.py`
    trains on.
    """
    ledger = records()
    out = [
        ...
        "",
        "**Derived from `Plan/runs/judgements.jsonl` by "
        "`python3 scripts/judgements.py --render`. Do not edit.**",
        "",
        "It exists so the decisions are searchable: qmd indexes markdown only, so "
        "the ledger itself is invisible to it. Ask the `decisions` collection by name.",
        "",
    ]
    for record in ledger:
        verdict, _ = replay(record)
        out += [
            f"## {record['id']} — {' / '.join(record.get('surfaces', []))}",
            "",
            f"**{record['decision']}** · {record.get('document', '?')} · "
            f"{record.get('at', '?')} · replay: `{verdict}`",
            "",
            f"- **rule:** {record['rule'] or '*none stated — still open*'}",
            f"- **mechanised by:** `{record.get('mechanised_by') or 'nothing'}`",
            f"- **features:** {', '.join(record.get('env_features', [])) or '—'}",
            "",
            f"**Question.** {record.get('goal', '')}",
            "",
            f"**What was done.** {record.get('action', '')}",
            "",
            f"**Result.** {record.get('result', '')}",
            "",
        ]
    RENDERED.write_text("\n".join(out), encoding="utf-8")
    print(f"wrote {RENDERED.relative_to(ROOT)} — {len(ledger)} judgements")
    return 0
...
def main(argv: list[str]) -> int:
    ...
    # Re-render on every run rather than on request. The markdown view is what
    # qmd can see, and a searchable copy of the ledger that lags behind it would
    # answer „what was decided?" with a decision that has since changed — the
    # same failure the state check exists for, in the one place a search would
    # not reveal it. `state.py` writes Plan/state.json the same way.
    if not only_open:
        render()
    if counts["DISAGREES"]:
