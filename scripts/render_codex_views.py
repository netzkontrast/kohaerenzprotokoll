...
A GLOSSARY, a MASTER-TIMELINE and an index of world rules are what the
repo's checks and commands read. Those facts already live in `Graph/` as
...
Never hand-edit the generated files: change the record in `Graph/nodes/*.jsonl`
and re-render. Archived codex entries are skipped.
"""
...
HEADER = (
    "<!-- GENERATED by scripts/render_codex_views.py from Graph/ — "
...
def nodes(g: kpgraph.Graph, label: str) -> list[dict]:
    """All nodes of `label` with their properties, in export order."""
    return g.nodes(label)


def edges(g: kpgraph.Graph, etype: str) -> list[tuple[int, int]]:
    return g.edges(etype)
...
def render_glossary(c: kpgraph.Graph) -> str:
    ...
    lines = [HEADER, "# Glossar — Kohärenz Protokoll (generiert)\n",
             ...
             "Die vollständigen Einträge liegen in `Graph/nodes/codex_entry.jsonl`; "
             "hier steht pro Eintrag Name, Slug, Trigger und "
...
def render_timeline(c: kpgraph.Graph) -> str:
...
def render_axioms(c: kpgraph.Graph) -> str:
    ...
    lines = [HEADER, "# Welt-Axiome — Kohärenz Protokoll (generiert)\n",
             ...
             "`…_welt-sensorik-drafting_2026-06-10.md`; Widersprüche prüft "
