...
def render_glossary_entry(entry: dict) -> str:
    ...
    content_words = _word_count("\n".join(content))
    if content_words > MAX_ENTRY_WORDS:
        raise ValueError(
            f"CodexEntry {entry.get('slug', '?')!r} has {content_words} words; "
            f"split the graph entity before rendering (maximum {MAX_ENTRY_WORDS})")
