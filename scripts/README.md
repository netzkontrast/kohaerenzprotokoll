...
| `chapter_sources.py` | Navigation for the chapter pages: each chapter's questions (written first, `questions/kap-NN.json`) asked of qmd's vectors one at a time, and the unread landed documents they return, fused by rank. `write` replaces each page's `## Questions for this chapter` and `## Candidate sources` sections; `run --terms` asks the same of every term page into the run only. A hit is a place to look, never a reading or a number. | `Plan/runs/qmd-chapters-2026-09-26/` |
