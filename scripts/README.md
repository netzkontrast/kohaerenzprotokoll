...
| `chapter_sources.py` | Navigation for the chapter pages: one qmd vector query per chapter page, built from its titles and quoted beats, and the unread landed documents it ranks. `write` replaces each page's `## Candidate sources` section; `run --terms` asks the same of every term page into the run only. A hit is a place to look, never a reading or a number. | `Plan/runs/qmd-chapters-2026-09-26/` |
