...
# Reconciliation 14 — `kohaerenz-protokoll-begriffe-und-konzepte-2026-06-10-md` against the wiki
...
| rule | pages |
|---|---|
| a `[K]` entry of §1, the physics the storyform document said was elsewhere, that defines a project construct | `dkt` · `kohaerenz-kernel` · `kollaps-kernel` · `atemporalitaet` · `drei-ontologische-schichten` |
| a `[K]` entry whose subject is an open conflict record | `hitze-polaritaetsregel` (C11) · `genesis` (C12) |

`genesis` gathers every read source's version of the beats — five of them — so
C12 can be read on one page.

## Readings — 43 pages

Every lookup hit, the thirteen Alters from its table, plus `landauer-signatur`,
`nexus` (J63) and `entropie`, reached by what the passages state.

## What it adds to the open records

**Twelve conflicts and three questions**, all but Q2 and Q4. The ones that move:
...
J68. The rest of the 64 were decided by J16, J21, J28, J47, J49, J56, J57, J60, J63.
