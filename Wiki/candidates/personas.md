---
term: Personas
status: candidate
sources: 1
readings: 1
conflict: none
ingested: ["guardians-und-kern-welten-konzept"]
aliases: ["Persona", "Bewusstseinsinstanzen"]
gathered: "2026-09-17"
---

# Personas

The consciousness instances the [[guardians|Guardians]] watch over. 18 occurrences.

## Reading — `guardians-und-kern-welten-konzept`, 2025-04-17

Glossed once, beside a second term in the same clause — [[cerberus|Cerberus]] protects
„der einzelnen Bewusstseinsinstanzen (Personas)" ^[L74]. That parenthesis is
the only definition offered.

Each Kern-Welt names the Persona that primarily experiences it:

| Persona | world | line |
|---|---|--:|
| Architekt | [[konstrukt-stadt|Konstrukt-Stadt]] | 35 |
| Echo | [[resonanz-landschaft|Resonanz-Landschaft]] | 60 |
| Wächter | [[grenzfeste|Grenzfeste]] | 85 |
| Funke · Sucher | [[moeglichkeits-garten|Möglichkeits-Garten]] | 124 |

**Six Persona names for four worlds**, because the [[moeglichkeits-garten|Möglichkeits-Garten]] has two.

## Not to be confused with the Modi

`Philosophen-Modus`, `Forscher-Modus` and `Magier-Modus` ^[L26, L51, L103, L115]
are referenced as an external framework already known, and are used to describe
a Guardian's *epistemology*, not a Persona. Seven occurrences, never defined
here.

## Open

How many Personas exist, whether each maps to one alter of [[kael|Kael]]'s DID, and how
`Bewusstseinsinstanz` differs from `Persona` if at all — the document treats
them as the same thing in one parenthesis and never returns to it.
