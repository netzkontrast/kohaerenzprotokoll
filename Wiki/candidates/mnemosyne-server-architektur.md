---
term: Mnemosyne-Server-Architektur
status: candidate
sources: 2
readings: 2
conflict: none
ingested: ["kohaerenz-protokoll-kernwelten-vollstaendig-2026-06-10-md", "kohaerenz-protokoll-begriffe-und-konzepte-2026-06-10-md"]
gathered: "2026-09-24"
---

# Mnemosyne-Server-Architektur

**The setting of Vortex 1 (Kap 35–36), in KW2 — and its image is not decided.**

## Reading — `kohaerenz-protokoll-kernwelten-vollstaendig-2026-06-10-md`, 2026-06-10

„Vortex-1-Setting" ^[kohaerenz-protokoll-kernwelten-vollstaendig-2026-06-10-md.md:L250], `[K]`. „Mnemosyne-Server-Architektur — offenes Bild." ^[kohaerenz-protokoll-kernwelten-vollstaendig-2026-06-10-md.md:L254]: „Das primäre Bild ist beim Vortex-1-Encoding festzulegen." ^[kohaerenz-protokoll-kernwelten-vollstaendig-2026-06-10-md.md:L254] Four
images are offered — Inselgruppe, Server-Halle, Memorialarchitektur,
Zirbeldrüse-Architektur (L258–L261) — and the open-points table repeats it (L938).

Also named as the Vortex-1 setting in the storyform-und-outline (L370), the
konsolidiertes Konzept (L496) and the Kapitel-Kompendium (L232), not attached.

## Reading — `kohaerenz-protokoll-begriffe-und-konzepte-2026-06-10-md`, 2026-06-10

Vortex 1, Beat 1: „AEGIS isoliert Kaels Fragmente in der
Mnemosyne-Server-Architektur" ^[kohaerenz-protokoll-begriffe-und-konzepte-2026-06-10-md.md:L595].
