...
Reference catalogue of canon-enforcement principles, epistemological
operating rules, metascience diagnostic filters, adversarial validation
protocols and anti-patterns. This is reference material, not a procedure:
read it when an audit fails, when a new idea conflicts with existing canon,
or when a contested decision needs stress-testing. `/kp-decide` links here.

> **Kohärenz Protokoll adaptation.** The foundational axiom of this universe is the Dual-Kernel-Theorie
(K₁ coherence / K₀ collapse; Coheronen atemporal, Erasonen make time; AEGIS
is K₀ believing itself K₁). "Canon source" means `Canon/` with
`kohaerenz-protokoll_storyform-und-outline_2026-06-10.md` normative; the lock
index is Canon welt-sensorik §12; the hard rules are R-1…R-10 (§10.1). The
"intentional ambiguities" the framework says never to resolve include the
Kap-40 reading duality and the ontological status of the Externe Ebene.
