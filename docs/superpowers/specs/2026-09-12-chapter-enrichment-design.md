...
| Graph/disk divergence | Pre-existing: the graph holds only ch 0's body and 400–1,000-char stubs for ch 1–40, so `scripts/materialize_manuscript.py` would destroy every drafted chapter. A guard is added to that script; the graph is not the source of truth for prose. |
