...
run). Two stages: chapter-scoped **canon locks** (lexical, free, no key —
`--locks-only` runs them alone) and the **seven editorial lenses** against
`tools/lit-critic/CANON.md` + `STYLE.md`. Writes
`Plan/quality/lit-critic/kap-NN.md`. Only `critical` findings block; the
`horizon` lens never does. A lock proves a forbidden word is absent — it can
never show that something required is missing. Walk the `lit-critic` skill
...
key it exits 2 (**never** a pass), though `--locks-only` runs the canon locks
free of charge and reports `LOCKS PASS` so it is not mistaken for a full run. Tests: `.lit-critic-src/.venv/bin/python -m
pytest tests/`.
