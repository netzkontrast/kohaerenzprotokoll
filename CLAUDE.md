...
dated May 2026 or later, 33 <!--state:sources.canon_era_landed--> landed, sixteen of them
read (documents 7 to 22).
...
**22 <!--state:documents.with_census--> have a term census** in `Sources/terms/`, **22
<!--state:documents.with_note--> have a note** in `Sources/notes/`, and **22
...
`kernkonzept`, five `plot-outline` and one `theorie-psychologie` — the last sixteen
from the canon era.
...
**326 <!--state:chapters.readings--> readings** from the eight read documents
that go chapter by chapter, one that names six chapters, one that names three, and the draft text of two; `Wiki/overview/` lays the chapters and the plot's
...
**The twenty-first is the Doppel-Klammer Abhandlung of 2026-05-08, and it argues
rather than decides.** A treatise on the frame of Kap 0 and Kap 40, „ein einziger
Atem in zwei Richtungen", ending in three `Setzungen` it asks to have confirmed — the
Vermittler-Stimme as Wir-AEGIS-plural, a warmth trace in Kap 0, the same shards at
both ends. It was one of the ten documents the scan had quoted, so five pages already
carried it; they now name its reconciliation. **It gave the Formel-Inversion its
page**, gathered from the eight read sources that name it: they write the two
sentences alike and part on where the second falls — Kap 39, the turn to Kap 40, or
Kap 40 itself. It counts four Genesis beats, only two of them in Kap 0 and the fourth
in Kap 39 (C12); puts warmth in Kap 0 as Juna's trace (C11); and gives Kap 40's last
line „laut Konzept" as „Wir tragen die Scherben", on the date both concept documents
read end on „Wir tragen die Welt". **It names `Mira` among the Wir's voices**, a name
in no other landed document (Q3). J94 makes `Doppel-Klammer` the Genesis-Klammer's
...
reading from it says how the draft renders a thing, voice by voice: the Funken-Ich's first
person, AEGIS in the third, Kap 40's Wir. It makes Kael the remainder the
Trennungsprotokoll cuts away and leaves in the Innere Weite (C12), gives the Konstrukt-Stadt
its twenty-one degrees and an ozone nobody can explain (C11), and has no knuckles in Kap 0,
where later sources put them (C10). **No pages**: its one defined thing, the Innere Weite, is
what three plans gloss as the Überwelt, and J95 makes it that page's alias. It writes the
Formel-Inversion's second sentence with a plural verb, which made `formel-inversion`'s lead
false. And its list repeated the nominative defect the briefing had asked about that
morning — five zeros — so briefing v12 has the reader ask `read.py --find` for a phrase
before listing it.

`Plan/runs/judgements.jsonl` holds **95 <!--state:judgements.total--> judgements**
...
is recorded in `Plan/runs/sweep.jsonl`: 36 <!--state:sweep.decided--> so far,
19 <!--state:sweep.readings--> of them readings the lookup had missed, and
...
**474 <!--state:wiki.relations--> links across
...
The 260 <!--state:wiki.unmarked--> mentions still unmarked are ones where every
...
conflicts, questions) and **2235 <!--state:graph.edges--> edges** (`links`,
...
Its evidence is every quotation on a term page: 3280 <!--state:graph.evidence-->
of them, **3280 <!--state:graph.evidence_verified--> verified** against their
...
links. It has 474 <!--state:wiki.relations-->.
...
**81 <!--state:pairs.labelled--> labelled pairs; `fold()` decides
