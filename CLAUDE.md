...
| `pairs.py` | one-term-or-two: a rule first (`fold()`, or the plural rule of decision 010), a model only on the residual, stratified folds, repeats, and every candidate asked the never-merge canaries. `report` splits every ledger row into merges found and **false merges**, by judgement id; `--evidence` adds the document lines code places (never to a free model); `final` compiles once and saves the program |
