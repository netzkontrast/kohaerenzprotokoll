...
**The eighth is the first `charaktere` document, and its one new page is a
split rather than a discovery.** 108 candidates across 21 full character
profiles — the richest shape read so far, richer than the seventh's own
sketch table — and still zero pages created from them: every proposed name
carries `oder ähnlich`, the same hedge that kept the fifth document at zero.
`zero-trust.md` splits off the existing `aegis-teilfunktionen.md` instead,
the moment that page's own pre-stated condition was finally met. Its own
most confident data breaks a strict one-alter-one-world reading directly —
two Alters share a world, a third is given two at once, both unhedged — and
it names a fourth, disjoint Alter roster against a fifth document's, zero
