...
vocabulary is its method, and its world lands on pages that exist. A second,
independent reading of it the same day (pull request #83) found C13 — whether
Köln 2026 lies beyond the simulation — which main took; its candidate list is kept
as a blind re-reading, F1 0.63 against the committed one.
...
`Plan/runs/judgements.jsonl` holds **84 <!--state:judgements.total--> judgements**
...
**420 <!--state:wiki.relations--> links across
93 <!--state:wiki.pages--> pages, 27 <!--state:wiki.orphans--> of them with
...
The 160 <!--state:wiki.unmarked--> mentions still unmarked are ones whose first
...
conflicts, questions) and **1617 <!--state:graph.edges--> edges** (`links`,
...
Its evidence is every quotation on a term page: 1877 <!--state:graph.evidence-->
of them, **1877 <!--state:graph.evidence_verified--> verified** against their
...
more (C14, C15); on the original nine the numbers were 40 and 58, and on
the eighteen before document 17 they were 45 and 65.
...
links. It has 420 <!--state:wiki.relations-->.
...
**71 <!--state:pairs.labelled--> labelled pairs; `fold()` decides
