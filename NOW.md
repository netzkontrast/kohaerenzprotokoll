...
`pairs.RULES["plural"]` decides 50 <!--state:pairs.plural_correct--> of
73 <!--state:pairs.labelled--> pairs where `fold()` decides
42 <!--state:pairs.fold_correct-->, with no false merge, no canary merged, no two
...
8. **Every reconciliation ends with `reconcile.py --sweep-open` printing
   ...
   29 <!--state:sweep.decided--> hits, 14 <!--state:sweep.readings--> of them
...
Next, by the open records: `kohaerenz-protokoll-konzept-master-md` (1123 lines, 2026-05-08) — `Ozon`, `Landauer`, `734`, `Flight`, `Knöchel`, `Be-er`/`Do-er` and `Erasure` all occur (`grep -ci`, orientation only), so it can speak to C8, C10, C11, C12, C14, C15 and Q5. The other session's named next, `kp-plot-konkretisierung-13-ideen-f1-faden-2026-06-10-md`, is left to it.
